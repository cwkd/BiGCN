...
def train_GCN(treeDic, x_test, x_train, TDdroprate, BUdroprate, lr, weight_decay, patience, n_epochs,
    ...
    try:
        for epoch in range(n_epochs):
            ...
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            save_path = os.path.join(save_dir, f'final_bigcn_f{fold}_i{iter}_e{epoch:05d}_l{loss:.5f}.pt')
            th.save(checkpoint, save_path)
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        save_path = os.path.join(save_dir, f'interrupt_bigcn_f{fold}_i{iter}_e{epoch:05d}_last.pt')
        th.save(checkpoint, save_path)
    # except:
